...
**Note:** `/chosen/path` in Bash rules must be the **expanded absolute path** (not `~/`), because Bash rules match against the actual command string. Use the same path as in `REMEMBER_BRAIN_PATH`.
...
- `permissions.allow`: append these rules if not already present, keep existing entries
